...
import type { BuilderProtocolResponse } from "@/types/protocol"
...
export async function getCachedBuilderProtocolResponse(requestId: string, protocolName: string) {
...
export async function cacheBuilderProtocolResponse(
  requestId: string,
  protocolName: string,
  response: BuilderProtocolResponse<Record<string, unknown>>
) {
...
export async function getBuilderProtocolCacheCount() {
  const records = await readCache()
