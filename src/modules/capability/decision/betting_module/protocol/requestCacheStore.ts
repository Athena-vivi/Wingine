...
export async function getCachedBettingProtocolResponse(requestId: string, protocolName: string) {
...
export async function cacheBettingProtocolResponse(
  requestId: string,
  protocolName: string,
  response: ProtocolResponse<Record<string, unknown>>
) {
...
export async function getBettingProtocolCacheCount() {
  const records = await readCache()
